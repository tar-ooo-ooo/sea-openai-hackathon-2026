...
export default function CareReport({ item, onReturnToChat }: { item: CaseData["drafts"][number] | CaseData["cases"][number]; onReturnToChat?: () => void }) {
  ...
  return <article className={styles.report}>
    ...
    <footer className={styles.reportFooter}><strong>申請前提醒</strong><p>請先核對這份紀錄。服務建議及資料整理不等於已送出、已受理或已核定；實際評估結果仍須由受理單位確認。</p>{onReturnToChat ? <button type="button" className="button secondary" onClick={onReturnToChat}>完成檢閱，返回對話</button> : <Link className="quiet-link" href="/cases">返回我的案件</Link>}{onReturnToChat && <p>返回對話不會送出申請，也不會記錄送件授權。</p>}</footer>
  </article>;
}
