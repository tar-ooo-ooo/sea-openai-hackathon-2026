...
import { isAllowedAdminOrigin, isAllowedOrigin } from "./allowed-origin.ts";
...
test("後台 API 只接受後台專用 origin", () => {
  ...
  try {
    ...
    assert.equal(isAllowedOrigin("http://localhost:3003"), true);
    assert.equal(isAllowedAdminOrigin("http://localhost:3003"), false);
